...
-- ??? AssetTest ??? ???? ??????
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'AssetTest')
BEGIN
    PRINT '??? ??? ????? ???????? AssetTest ????????...'
    ...
    SELECT @SQL = @SQL + 'KILL ' + CAST(session_id AS VARCHAR(10)) + '; '
    FROM sys.dm_exec_sessions
    WHERE database_id = DB_ID('AssetTest') AND session_id <> @@SPID
    
    IF LEN(@SQL) > 0
        EXEC(@SQL)
    
    ALTER DATABASE AssetTest SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE AssetTest;
    PRINT '? ?? ??? AssetTest ???????'
END
...
-- ???? ??? ??? backup ??????
SET @BackupPath = 'C:\Temp\Assets_Complete_Backup_' + 
...
WITH 
    ...
    COMPRESSION,
    CHECKSUM,
    NAME = ''Assets Complete Database Backup'',
    DESCRIPTION = ''Full backup including all objects, data, and relationships'',
    STATS = 5'
...
-- ????? ??????? ???????
DECLARE @DataFileName NVARCHAR(500) = @DataPath + 'AssetTest.mdf'
DECLARE @LogFileName NVARCHAR(500) = @LogPath + 'AssetTest_Log.ldf'

PRINT '?? ??? ???????? ??????: ' + @DataFileName
PRINT '?? ??? ????? ??????: ' + @LogFileName

-- ???? ???? RESTORE ?? ???? ???????
DECLARE @RestoreSQL NVARCHAR(MAX) = 'RESTORE DATABASE [AssetTest] FROM DISK = ''' + @BackupPath + ''' WITH '
...
-- ????? ????? ????? ???????? ????????
ALTER DATABASE AssetTest SET RECOVERY SIMPLE;
ALTER DATABASE AssetTest SET AUTO_SHRINK OFF;
ALTER DATABASE AssetTest SET AUTO_UPDATE_STATISTICS ON;
ALTER DATABASE AssetTest SET PAGE_VERIFY CHECKSUM;
...
PRINT 'Data Source=10.0.0.17;Initial Catalog=AssetTest;User ID=sa;Password=Dur@123456;Connect Timeout=30;Encrypt=True;Trust Server Certificate=True;Application Intent=ReadWrite;Multi Subnet Failover=False'
PRINT ''

-- ?????? ??????? ?? ???? ????? ????????
IF DB_ID('AssetTest') IS NOT NULL
BEGIN
    PRINT '?? ???? ????? ????????: ? ????? ??????'
    PRINT '?? ??? ????? ????????: ' + 
          CAST(CAST((SELECT SUM(size) * 8.0 / 1024 FROM AssetTest.sys.database_files) AS DECIMAL(10,2)) AS VARCHAR(20)) + ' MB'
    
    PRINT ''
    PRINT '?? ??????? ???????:'
    PRINT '1. ? ????? appsettings.json ???????? TestConnection'
    PRINT '2. ? ?????? ??????? ?????? ???????? ???????'
    PRINT '3. ? ?????? ?? ??? ???? ????? ???????'
    PRINT '4. ? ????? ???????? ?????'
    PRINT ''
